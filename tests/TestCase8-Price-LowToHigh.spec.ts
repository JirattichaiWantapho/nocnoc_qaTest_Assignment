...
test('testcase_Price-LowToHigh', async ({ page }) => {
  ...
  for(let i = 0; i < count; i++){
    ...
    try {
      await expect(NowPrice).toBeGreaterThanOrEqual(LowPrice);
      LowPrice = NowPrice;
    } catch (error) {
      console.error(`Item ${i + 1} : ${NowPrice} Price less than Item ${i}: ${LowPrice}`);
    }
  }
